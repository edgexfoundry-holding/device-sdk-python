...
class TLSManager:
    ...
    Features:
    - Certificate loading and validation
    - Automatic certificate rotation with hot-reload
    - Certificate expiration monitoring
    - mTLS support for mutual authentication
    ...
    def __init__(
        ...
        Args:
            server_config: Server TLS configuration (for HTTPS server)
            client_config: Client TLS configuration (for outgoing connections)
            auto_renewal: Enable automatic certificate renewal
            renewal_threshold_days: Days before expiry to trigger renewal
            check_interval_hours: Interval for certificate expiration checks
    ...
    def reload_certificates(self) -> bool:
        ...
        with self._lock:
            try:
                ...
                # Restore old contexts on failure
    ...
    def _load_certificate(self, cert_file: str) -> x509.Certificate:
    ...
    def get_certificate_info(self, cert_file: str) -> Optional[CertificateInfo]:
        ...
        try:
            ...
            return CertificateInfo(
                ...
                is_ca=cert.extensions.get_extension_for_oid(x509.oid.ExtensionOID.BASIC_CONSTRAINTS).value.ca if cert.extensions.get_extension_for_oid(x509.oid.ExtensionOID.BASIC_CONSTRAINTS) else False,
    ...
    def check_and_renew(self) -> bool:
        ...
        for cert_file, info in results.items():
            if info["status"] in ("expired", "expiring_soon"):
                self._logger.warning("Certificate %s is %s (expires in %d days)", 
    ...
    def _attempt_renewal(self, cert_file: str) -> bool:
        ...
        # the certificate authority (Let's Encrypt, OpenBao, etc.)
        self._logger.info("Certificate renewal attempted for %s", cert_file)
        # Implementation would call ACME client, Vault, OpenBao, etc.
        # For now, just reload certificates if they were updated externally
        try:
    ...
    def create_self_signed_cert(
        ...
        builder = x509.CertificateBuilder().subject_name(
            ...
            x509.SubjectAlternativeName([x509.DNSName(d) for d in (san or [])]),
            critical=False,
        )
